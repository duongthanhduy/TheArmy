...
public class WizardBullet : MonoBehaviour
{
    ...
    private IEnumerator OnDealDmg() {
        ...
        {
            //Debug.LogError($"{overlapColliders[i].gameObject.name}");
            if (overlapColliders[i].CompareTag("brick")) {
                Brick _brick = overlapColliders[i].gameObject.GetComponent<Brick>();
                bullet.DealDamageToMono(_brick);
            }
            else if (overlapColliders[i].CompareTag("entity")) {
